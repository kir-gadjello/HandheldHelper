...
class StatefulRPCProcessor<T, U> {
...
typedef ProcessorFactory<T, U> = StatefulRPCProcessor<T, U> Function();
...
class IsolateRpcService<T, U> implements IsolateRpc<T, U> {
  ...
  IsolateRpcService._({required ProcessorFactory<T, U> processorFactory, this.id = 0, this.debugName, this.logger}) {
    ...
    this._isolate = Isolate.spawn<_IsolateProcessor>(
        _IsolateProcessor.entryPoint,
        _IsolateProcessor<T, U>(processorFactory: processorFactory, port: resultPort.sendPort),
...
class _IsolateProcessor<T, U> {
  ...
  final ProcessorFactory<T, U> processorFactory;

  _IsolateProcessor({required ProcessorFactory<T, U> this.processorFactory, required this.port, this.logger});

  static void entryPoint(self) {
    self.process();
  }

  /// Process each incoming RpcRequest using [processor]
  void process() {
    StatefulRPCProcessor _custom_processor = processorFactory();
    ...
    resultPort.handler = (message) async {
      if (message is _RpcRequest) {
        ...
        try {
          final U result = await _custom_processor.process(data);
...
abstract class IsolateRpc<T, U> {
  ...
  static IsolateRpc<T, U> pool<T, U>(
      {required int size, required ProcessorFactory<T, U> processorFactory, String debugNamePrefix = "rpc_pool", Logger? logger}) {
    List<IsolateRpc<T, U>> services = [];
    for (var i = 0; i < size; i++) {
      services.add(single(id: i, processorFactory: processorFactory, debugName: "${debugNamePrefix}_$i", logger: logger));
    }
  ...
  static IsolateRpc<T, U> single<T, U>(
      {required ProcessorFactory<T, U> processorFactory, int id = 0, String? debugName, Logger? logger}) {
    return IsolateRpcService._(processorFactory: processorFactory, id: id, debugName: debugName, logger: logger);
  }

...
cd ../llama.cpp || exit;
rm -rf build-arm64-v8a-opencl-static;
mkdir build-arm64-v8a-opencl-static;
cd build-arm64-v8a-opencl-static || exit;
...
-DCLBlast_DIR=./lib/cmake/CLBlast -DLLAMA_CLBLAST=1 \
...
cp ./build-arm64-v8a-opencl-static/examples/server/librpcserver.so ../handheld_helper/native/arm64-v8a/librpcserver-opencl.so
cp ./examples/server/rpcserver.h ../handheld_helper/native/
cp ./examples/server/rpcserver.h ../handheld_helper/
# cp ./build-arm64-v8a-opencl-static/libllama.so ../handheld_helper/native/arm64-v8a

cd ../handheld_helper || exit;

mkdir -p android/app/src/main/jniLibs/arm64-v8a
cp -R native/arm64-v8a/librpcserver-opencl.so android/app/src/main/jniLibs/arm64-v8a/librpcserver-opencl.so
